...
newString& newString::operator+=(const newString& other){
    ...
    newString temp(other.c_str()); //In case of s1 += s1;
    this->append(temp.c_str());
    return *this;
}

newString& newString::operator+=(const char* stringToAppend){
    newString temp(stringToAppend);//In case of s1 += s1.c_str();
    this->append(temp.c_str()); 
    return *this;
}

newString newString::operator+(const newString& other){ 
    newString temp(this->c_str()); //In case of s2 = s1 + s1
    temp.append(other.c_str());
    return temp;
}

newString newString::operator+(const char* stringToAppend){
    newString temp(this->c_str()); //In case of s2 = s1 + s1.c_str()
